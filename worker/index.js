...
function googleRssUrl(queryWords) {
  ...
  return `https://news.google.com/rss/search?q=${encodeURIComponent(queryWords + " after:" + after)}&hl=zh-CN&gl=CN&ceid=CN:zh-Hans`;
}

const SOURCE_FEEDS = [
  // Google News RSS：动态日期过滤，只返回最近7天
  { name: "Google新闻-期货监管", get url() { return googleRssUrl("期货 证监会 交易所 监管 公告"); }, type: "google-rss", category: "regulation", keywords: ["期货", "证监会", "交易所", "监管", "公告", "处罚", "保证金", "涨跌停", "手续费"] },
  { name: "Google新闻-期货公司", get url() { return googleRssUrl("期货公司 风险管理 合规 期货行业"); }, type: "google-rss", category: "peer", keywords: ["期货公司", "风险管理", "合规", "期货行业", "处罚", "高管", "从业", "协会"] },
  { name: "Google新闻-品种行情", get url() { return googleRssUrl("期货 黄金 原油 铜 螺纹 豆粕 大涨 大跌"); }, type: "google-rss", category: "variety", keywords: ["期货", "黄金", "原油", "铜", "螺纹", "豆粕", "大涨", "大跌", "涨停", "跌停", "行情"] },
  { name: "Google新闻-宏观金融", get url() { return googleRssUrl("央行 美联储 利率 汇率 期货"); }, type: "google-rss", category: "finance", keywords: ["央行", "美联储", "利率", "汇率", "货币政策", "期货", "宏观", "经济"] },
  { name: "Google新闻-衍生品",  get url() { return googleRssUrl("衍生品 期货 基差 仓单 持仓 多空"); }, type: "google-rss", category: "variety", keywords: ["衍生品", "期货", "基差", "仓单", "持仓", "多空", "库存"] },
  // 权威官方 HTML
...
function isRecent(item) {
  ...
  if (t > Date.now() + 3_600_000) return false;
...
async function buildLiveDashboard() {
  ...
  const dashboard = buildDashboard(freshItems);
  dashboard.health = {
